...
class Customer
{
    ...
    function setCustomerEmail($CustomerEmail)
    {
        ...
        else if ($this->findByEmail($CustomerEmail))
            $this->errors["CustomerEmailError"] = "<p>Email already exist.</p>";
    }
    ...
    {
        if (isset($properties["CustomerID"]))
            $this->setCustomerID($properties["CustomerID"]);
        if (isset($properties["ProvinceID"]))
            $this->setProvinceID($properties["ProvinceID"]);
        if (isset($properties["CustomerName"]))
            $this->setCustomerName($properties["CustomerName"]);
        if (isset($properties["CustomerEmail"]))
            $this->setCustomerEmail($properties["CustomerEmail"]);
        if (isset($properties["CustomerPhone"]))
            $this->setCustomerPhone($properties["CustomerPhone"]);
        if (isset($properties["CustomerAddress"]))
            $this->setCustomerAddress($properties["CustomerAddress"]);
        if (isset($properties["CustomerCity"]))
            $this->setCustomerCity($properties["CustomerCity"]);
        if (isset($properties["PasswordHash"]))
            $this->setPasswordHash($properties["PasswordHash"]);
    ...
    function findByEmail($Email)
    {
        ...
        $stmt = $pdo->prepare("select * from customers where 
                ...
                ");
        $stmt->execute(["CustomerEmail" => $Email]);
        if ($stmt->rowCount() > 0)
            return true;
        else
            return false;
